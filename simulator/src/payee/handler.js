...
const partiesHost = process.env.PARTIES_HOST || 'ml-api-adapter.local'
const quotesHost = process.env.QUOTES_HOST || 'ml-api-adapter.local'
const transfersHost = process.env.TRANSFERS_HOST || 'ml-api-adapter.local'
  ...
  (async function () {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Failed to send. Result: ${JSON.stringify(res, null, 2)}`)
